...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .sidebar {
      width: 250px; background-color: #f57c00; color: white;
      padding: 20px; display: flex; flex-direction: column;
      gap: 15px; transition: all 0.3s ease-in-out;
    }
    .btn-evakuasi {
      background-color: white; color: #f57c00; font-weight: bold;
    }
    #map { flex-grow: 1; height: 100%; transition: filter 0.3s ease-in-out; }
    #map.blur-bg { filter: blur(5px); }
    #infoBox {
      padding: 10px; background-color: #f8f9fa;
      border-top: 1px solid #ddd; text-align: center;
    }
    .d-flex-column { display: flex; flex-direction: column; flex-grow: 1; }
    @media (max-width: 768px) {
      ...
      .sidebar .btn { padding: 6px 10px; font-size: 14px; }
      #infoBox { font-size: 14px; padding: 8px; }
      #map .leaflet-marker-icon { transform: scale(0.8); }
    }
  </style>
</head>
<body>
...
L.marker(titikUser, {
  icon: L.icon({ iconUrl: 'https://cdn-icons-png.flaticon.com/512/64/64113.png', iconSize: [26, 26] })
...
const polygonLayer = L.geoJSON(polygonData, {
  style: f => ({ color: f.properties.color || 'red', fillColor: f.properties.color || 'red', fillOpacity: 0.4 })
}).addTo(map);
...
markerData.forEach((m, i) => {
  ...
  evakuasiPoints.push({ latlng, nama: m.nama });
...
document.getElementById('startBtn').addEventListener('click', () => {
  ...
  const safeEvakuasi = evakuasiPoints.filter(p => !isInsideAnyDisasterPolygon(p.latlng));

  if (!safeEvakuasi.length) {
  ...
  safeEvakuasi.forEach(p => {
    const distance = titikUser.distanceTo(p.latlng);
    if (distance < shortestDistance) {
      shortestDistance = distance;
      bestRoute = [titikUser, p.latlng];
      chosenTarget = p.latlng;
    }
  });

  if (!bestRoute) {
    info.innerHTML = '<span class="text-danger">⚠️ Jalur tidak ditemukan.</span>';
    return;
  }
  ...
  info.innerHTML = `✅ Jalur aman ditampilkan.<br>Jarak: <strong>${(shortestDistance / 1000).toFixed(2)} km</strong>`;
});
